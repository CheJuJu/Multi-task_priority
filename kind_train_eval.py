...
def train(config, model, train_iter, dev_iter, test_iter):
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            ...
            model.zero_grad()
            # l1=MultiCEFocalLoss(class_num = 3,alpha=torch.tensor([0.4,0.2,0.4]))
            l2=MultiCEFocalLoss(class_num = 3,alpha=torch.tensor([7/82,20/82,55/82]))
            loss = l2(outputs.cpu(), labels[1].cpu())
            loss.backward()
            optimizer.step()
            if total_batch % 100 == 0:
                # 每多少轮输出在训练集和验证集上的效果
                true2 = labels[1].data.cpu()
                predic2 = torch.max(outputs.data, 1)[1].cpu() 
                train_acc2 = metrics.accuracy_score(true2, predic2)
                dev_acc2, dev_loss = evaluate(config, model, dev_iter)
                ...
                msg = 'Iter: {0:>6},  Train Loss: {1:>5.2},   Train Acc_kind: {2:>6.2%},  Val Loss: {3:>5.2},  Val Acc_kind: {4:>6.2%},  Time: {5} {6}'
                print(msg.format(total_batch, loss.item(),  train_acc2, dev_loss,  dev_acc2, time_dif, improve))
                model.train()
            total_batch += 1
...
def test(config, model, test_iter,log_path):
    ...
    model.eval()
    start_time = time.time()
    test_acc2, test_loss, test_report2, test_confusion2 = evaluate(config, model, test_iter, test=True)
    msg = 'Test Loss: {0:>5.2},  Test Acc_kind: {1:>6.2%}'
    print(msg.format(test_loss, test_acc2))
    ...
    file.write(msg.format(test_loss, test_acc2)+'\n\n')
...
def evaluate(config, model, data_iter, test=False):
    ...
    with torch.no_grad():
        for texts, labels in data_iter:
            ...
            predic2 = torch.max(outputs.data, 1)[1].cpu().numpy()
    ...
    if test:
        ...
        return acc2, loss_total / len(data_iter), report2, confusion2
    return  acc2, loss_total / len(data_iter)
